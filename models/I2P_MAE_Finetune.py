...
class I2P_MAE_ModelNet40(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[I2P_MAE_ModelNet40]', logger ='I2P_MAE_ModelNet40')
        self.config = config
...
@MODELS.register_module()
class I2P_MAE_ScanObjectNN(nn.Module):

    def __init__(self, config):
        super().__init__()
        print_log(f'[I2P_MAE_ScanObjectNN]', logger ='I2P_MAE_ScanObjectNN')
        self.config = config
    ...
    def load_model_from_ckpt(self, ckpt_path):
        ...
        incompatible = self.load_state_dict(state_dict['base_model'], strict=False)
        if incompatible.missing_keys:
            print_log('missing_keys', logger='I2P_MAE_ScanObjectNN')
            print_log(
                get_missing_parameters_message(incompatible.missing_keys),
                logger='I2P_MAE_ScanObjectNN'
            )
        if incompatible.unexpected_keys:
            print_log('unexpected_keys', logger='I2P_MAE_ScanObjectNN')
            print_log(
                get_unexpected_parameters_message(incompatible.unexpected_keys),
                logger='I2P_MAE_ScanObjectNN'
            )
